...
**Important:** the raw historical output/trace ZIP archives and supplementary figure-data CSVs are housed in the research archive, not duplicated in the selected GitHub source package. The authentic v8 predecessor was subsequently supplied by the author, verified against the original v9 checkpoint, and used to rerun the full synthetic v9 computation. The original v8 binary is retained outside this GitHub source checkout; see [v8 recovery and v9 rerun](docs/V8_RECOVERY_AND_V9_RERUN.md). This completed transfer does not by itself imply new independent validation or justify a release declaring all experiments reproduced.
